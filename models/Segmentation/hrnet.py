...
class HRNet(nn.Module):
    ...
    def __init__(self, in_channels=3, num_classes=1, hr_cfg='w48', with_upsample=True, pretrained='/data02/ybn1/Projects/comparison/segmentation/torch_cache/hrnetv2_w48_imagenet_pretrained.pth'):
    ...
    def _init_weights(self, pretrained='/home/zzl/codes/interRSSeg/pre_weights/hrnetv2_w48_imagenet_pretrained.pth'):
...
class HRNetSeg(nn.Module):
    ...
    def __init__(self, in_channels=3, num_classes=1, hr_cfg='w48', with_upsample=False):
        super(HRNetSeg, self).__init__()

        self.baseline = HRNet(in_channels=in_channels, num_classes=num_classes, hr_cfg=hr_cfg,
                              with_upsample=with_upsample)
        input_channels = self.baseline.input_channels
        ...
        if num_classes == 1:
            self.pre_class = nn.Sequential(
                nn.Conv2d(48, num_classes, 1, 1, 0),
                nn.Sigmoid()
            )
        else:
            self.pre_class = nn.Sequential(
    ...
    def base_forward(self, x):
        _, c, m, n = x.shape
        binary, f = self.baseline(x)

        fea_up = self.deconv(torch.cat([f, binary], dim=1))
        fea_re = self.res_block(fea_up)
        building_res = self.pre_class(fea_re)

        building_res = F.upsample(input=building_res, size=(m, n), mode='bilinear', align_corners=True)
        binary = F.upsample(input=binary, size=(m, n), mode='bilinear', align_corners=True)

        return building_res, binary

    def forward(self, x):
        return self.base_forward(x)

    def _init_weights(self,
                     pretrained='/home/zzl/codes/interRSSeg/pre_weights/hrnetv2_w48_imagenet_pretrained.pth',
                     model_pretrained=''):
        # logger.info('=> init weights from normal distribution')
        for m in self.modules():
            if isinstance(m, nn.Conv2d):
        ...
        if os.path.isfile(pretrained):
            ...
            model_dict.update(pretrained_dict)
            print(f'load-weight:{num}/{len(pre_weight.keys())}')
            self.baseline.load_state_dict(model_dict)
...
if __name__ == '__main__':
    ...
    # model.init_weights("../pre_weights_imagenet/higher_hrnet.pkl")
    inp = torch.randn(2, 3, 512, 512).cuda()
    check_map = torch.randn(2, 1, 512, 512).cuda()
    # inp = torch.cat((inp, check_map), dim=1)
