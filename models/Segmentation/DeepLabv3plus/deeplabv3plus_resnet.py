...
from models.Segmentation.DeepLabv3plus.oprations import ASPP_module
from models.Segmentation.DeepLabv3plus.resnet import *


class DeeplabV3Plus(ResNet):

    def __init__(self,in_channel, n_class, block, layers, pyramids, grids, output_stride=16):
        self.inplanes = 64
        ...
        # self.head = nn.Conv2d(256, n_class, kernel_size=1, stride=1)
        if n_class == 1:
            # self.head = nn.Sequential(
            #     nn.Conv2d(256, n_class, 1, 1, 0),
            #     nn.Sigmoid()
            # )
            self.head = nn.Sequential(
                nn.Conv2d(256, n_class, 1, 1, 0),
    ...
    def forward(self, input, vis_featurmap=False):
        x = self.conv1(input)
        x = self.bn1(x)
        x = self.relu(x)
        x = self.maxpool(x)
        ...
        f = self._conv3(x)
        if vis_featurmap:
            x_vis = F.interpolate(f, size=(512, 512), mode='bilinear', align_corners=False)
...
def resnet101_deeplabv3p(in_channel=3, n_class=1, output_stride=16, pretrained=True):
    if output_stride == 16:
        pyramids = [1, 6, 12, 18]
        grids = [1, 2, 4]
    elif output_stride == 8:
    ...
    model = DeeplabV3Plus(in_channel=in_channel,n_class=n_class, block=Bottleneck, layers=[3, 4, 23, 3],
...
def init_model(model):
    ...
    old_conv1_weight = pretrain_dict['conv1.weight']
    channel = state_dict['conv1.weight'].size()[1]
    for i in range(0, channel):
        state_dict['conv1.weight'][:, i, :, :] = old_conv1_weight[:, i % 3, :, :]

    for k, v in pretrain_dict.items():
        if k in state_dict.keys() and k != 'conv1.weight':
            model_dict[k] = v
            # print(k)
...
if __name__ == '__main__':
    model = resnet101_deeplabv3p(in_channel=3, n_class=1, output_stride=8, pretrained=True)
