...
from models.SAM.MaskDecoder import MaskDecoder, MaskDecoderHQ_Assess
...
def build_sam_decoder(args,encoder_embed_dim, prompt_embed_dim, cls='baseline'):
    if cls == 'baseline':
        ...
        mask_decoder = MaskDecoderHQ_Assess(
            num_multimask_outputs=args.class_num,
            transformer=TwoWayTransformer(
                depth=2,
                embedding_dim=prompt_embed_dim,
...
def _build_sam(
    ...
    mask_decoder = build_sam_decoder(args,encoder_embed_dim, prompt_embed_dim, cls='HQ_Assess')
    ...
    if checkpoint is not None:
        ...
        sam = load_ckpt(checkpoint, sam, dp_mode=args.dp_mode)
