...
from models.loss import HDNet_RCF_edge_criterion, BinSegLoss
...
def run_train(epoch, train_step, args, scaler):
    ...
    for i, batch in enumerate(args.train_loader):
        ...
        currpt_mask = batch['currpted_mask'].to(device)
        ...
        with torch.autocast(device_type='cuda', dtype=torch.float32):
            ...
            # mask_pred = torch.cat([mask_pred, currpt_mask], dim=1)
            # mask_pred = currpt_mask
            ...
            # if dist.get_rank() == 0:
            #     check_aug(image, edge_map_true, mask_pred, shuffled_name_list)
            batch['images'] = image
            ...
            # batch_out['init_preds']['boundary_pred'] = boundary_mask_pred.squeeze(1)
            ...
            # if dist.get_rank() == 0:
            #     color_mapping = np.array([l[0] for l in legend_data],dtype=np.uint8)
            #     assess_true = gt.squeeze(1).detach().cpu().numpy().astype(np.uint8)
            #     assess_true = color_mapping[assess_true]
            #     save_aug(image, mask_true, mask_pred, edge_map_true, assess_true, args.vis_root, extended_name_list)
...
def run_val(epoch, val_step, best_loss, args):
    with torch.no_grad():
        ...
        # if not args.configs['Experiment']['evaluate'] and dist.get_rank() == 0:
        #     args.logger.write_scalars({
